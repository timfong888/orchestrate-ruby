...
class CollectionGeoQueriesTest < MiniTest::Unit::TestCase
  ...
  def test_near_search_without_units
    @query = @near_query
    results = @items.near("location", 12, 56, 1).execute.each.map{|i| i }
  ...
  def test_near_search_with_units
    @query = @near_query
    results = @items.near("location", 12, 56, 1, 'km').execute.each.map{|i| i }
    assert_equal 10, results.length
    results.each_with_index do |item, idx|
      assert_in_delta (@total-idx/@total * 5.0), item[0], 0.005
      assert_equal "item-#{idx}", item[1].key
  ...
  def test_near_search_with_sort
    @query = @near_query
    results = @items.near("location", 12, 56, 1, 'km').order(:location).execute.each.map{|i| i }
    assert_equal 10, results.length
    results.each_with_index do |item, idx|
      assert_in_delta (@total-idx/@total * 5.0), item[0], 0.005
      assert_equal "item-#{idx}", item[1].key
  ...
  def test_in_bounding_box
    @query = @in_query
    results = @items.in("location", {north:12, east:57, south:12, west:56}).execute.each.map{|i| i }
  ...
  def test_in_bounding_box_with_sort
    @query = @in_query
    results = @items.in("location", {north:12, east:57, south:12, west:56}).order(:location).execute.each.map{|i| i }
    assert_equal 12, results.length
    results.each_with_index do |item, idx|
      assert_in_delta (@total-idx/@total * 5.0), item[0], 0.005
      assert_equal "item-#{idx}", item[1].key
